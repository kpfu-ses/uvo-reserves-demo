...
def get_wells_list(services_str, run_id):
    ...
    if ('1' in services) or ('3' in services):
        ...
        result2 = result
    if '2' not in services:
        result = result2
    result_wells = list(set(result2).intersection(result))
    result_wells = [(well.id, well.name) for well in result_wells]
    return result_wells
